...
export class DiarioBordoController {
    ...
    async calcularDistancia(req, res) {
        try {
            ...
            const getCoords = async (id) => {
                ...
                if (id.startsWith('LEGACY_')) {
                    ...
                    if (escola?.latitude && escola?.longitude)
                        return { lat: escola.latitude, lng: escola.longitude };
                    return null;
                }
            ...
            if (km === 0) {
                km = calcularDistanciaHaversine(coordsOrigem.lat, coordsOrigem.lng, coordsDestino.lat, coordsDestino.lng);
            }
    ...
    async excluir(req, res) {
        try {
            ...
            if (!usuarioId)
                return res.status(401).json({ error: 'Usuário não autenticado.' });
            if (!data)
                return res.status(400).json({ error: 'A data é obrigatória para exclusão.' });
