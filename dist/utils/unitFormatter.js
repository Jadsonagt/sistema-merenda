 ...
 */
const mapToLogisticUnit = (baseUnit) => {
    if (!baseUnit)
        return 'UN';
 ...
 */
export const formatLogisticValue = (diff, baseUnit, tamanhoEmbalagem) => {
    const logisticUnit = mapToLogisticUnit(baseUnit);
    const detail = tamanhoEmbalagem && tamanhoEmbalagem > 0
 ...
 */
export const formatVencimentoLabel = (dias) => `Vence em ${dias} dia${dias !== 1 ? 's' : ''}`;
