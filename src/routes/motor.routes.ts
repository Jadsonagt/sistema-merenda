 ...
 */
motorRoutes.post('/processar-dia',
  verificarToken,
  permitirRoles(['ADMIN']),
  motorController.processarDia
);

/**
 * Lista o histórico de processamentos realizados
 */
motorRoutes.get('/logs',
  verificarToken,
  permitirRoles(['ADMIN', 'SUPERVISORA']),
  motorController.listarLogs
);

/**
 * Previsão de Necessidade de Compras
 */
motorRoutes.post('/prever-compras',
  verificarToken,
  permitirRoles(['ADMIN', 'SUPERVISORA']),
  motorComprasController.preverCompras
);

motorRoutes.post('/pedido-mensal-consolidado',
  verificarToken,
  permitirRoles(['ADMIN', 'SUPERVISORA']),
  motorComprasController.exportarExcelConsolidado
);
